...
<?php  endif; ?>
  ...
  <tr>
      ...
      $doc->addScriptOptions('ComponentConfig' , $this->config ) ;
      JLoader::registerNamespace( 'GNZ11' , JPATH_LIBRARIES . '/GNZ11' , $reset = false , $prepend = false , $type = 'psr4' );
      $GNZ11_js =  \GNZ11\Core\Js::instance();
      $doc = JFactory::getDocument();
      $Jpro = $doc->getScriptOptions('Jpro') ;
      $Jpro['load'][] = [
          'u' => '/administrator/components/com_excel2js/assets/js/impotToLine.Core.js' , // Путь к файлу
...
<script>
    ...
    jQuery(function () {
        var $ = jQuery;
        var ComponentConfig = Joomla.getOptions('ComponentConfig');
        ...
        function start_import() {
            ...
            var importToLineObj = new window.impotToLine();
            ...
            // Показать кнопку прирвать !
            abort_button.show('slide', {}, 1500);

            if (ComponentConfig.line_import_on) {
                importToLineObj.ajaxImportSubmit($import_form)
        ...
        function showResponse(responseText, statusText) {
            ...
            abort_button.hide('explode', {}, 1500);
            reimport_counter = 0;
...
<fieldset class="panelform" style="width:800px;margin: 10px auto;">
    ...
    <form id="import_form" action="index.php" method="POST" enctype="multipart/form-data">
        <table style="border-collapse: collapse" id="uploaded_files_table" class="tablesorter">
            ...
            <?php foreach ($this->uploaded_files as $key => $f): ?>
                <tr id="row_<?php echo $key ?>">
                    <td><input name="uploaded_file[]" id="uploaded_file_<?php echo $key ?>" type="checkbox"
                               value="<?php echo $f->file ?>" style="margin-left: 14px"></td>
                    ...
                    <td><a href="index.php?option=com_excel2js&task=download&file=<?php echo $f->file ?>"><img
                                    src="<?php echo JURI::base() ?>/components/com_excel2js/assets/images/download.png"
                                    width="16" height="16" alt=""></a></td>
                    <td><img style="cursor: pointer" rel="<?php echo $key ?>" file="<?php echo $f->file ?>"
                             class="delete"
                             src="<?php echo JURI::base() ?>/components/com_excel2js/assets/images/delete.png"
        ...
        <ul class="adminformlist" style="text-align: center; margin: 10px auto; width: 200px">
            <li>
                <?= JHTML::tooltip('Для увеличения лимита, Вам необходимо увеличить параметры <b>post_max_size</b> и <b>upload_max_filesize</b> в настройках сервера', 'Максимальный размер zip-архива - ' . $max_size, '', '<span style="color: #CC6600; font-weight: bold;text-decoration:none; border-bottom:#F00 1px dashed;">Максимальный размер zip-архива - ' . $max_size . '</span>'); ?>
                <label class="wide"
                       for="zip_file"><?php echo JText::_('ENTER_YOUR_ZIP_FILE_WITH_THE_IMAGES_ON_YOUR_COMPUTER') ?>
                    :</label>&nbsp;<input id="zip_file" name="zip_file" type="file" size="30"/>
                <br/></li>
            <br clear="both"/>
        ...
        <ul class="adminformlist" style="text-align: center; margin: 10px auto; width: 400px">
            <li style="width: 400px;margin: 0 auto">
                <input id="show_results" name="show_results" type="checkbox"
                       value="1" <?php echo $show_results ? 'checked="checked"' : '' ?> >&nbsp;
                <label for="show_results"
                       style="width:380px!important;max-width:380px!important;float:none!important;display: inline;line-height: 21px;">Вывести
                    таблицу со всеми товарами после окончания</label>
            </li>
            <br>
            <li style="width: 110px;margin: 0 auto">
                <input style="float: none" type="button" class="btn btn-success"
                       id="import_button" value="<?php echo JText::_('START_IMPORT') ?>"/>
            </li>

            <li style="width: 110px;margin: 0 auto">
                <input style="float: none;display:none" type="button" class="btn btn-success" id="abort_button"
                       value="<?php echo JText::_('ABORT_IMPORT') ?>"/>
            </li>
        </ul>
    ...
    <h3 id="import_started" style="display: none" align="center">
        <?php echo JText::_('IMPORT_STARTED') ?>...</h3>
    <div id="statistics">
